...
- **`.aiko` - источник истины** - карточки, workflow, артефакты и память лежат в файлах `.aiko`;
  глобальная база SQLite хранит реестр проектов, историю запусков, журнал событий и поисковые индексы.
  `reindex` восстанавливает индексы из `.aiko`, но история запусков и журнал есть только в базе, поэтому
  её не удаляют
- **Безопасность по умолчанию** - привязка только на loopback, проверка `Host`/`Origin` против
  DNS-rebinding и любого origin, кроме собственной страницы демона, MCP только по bearer-токену, без
  фреймов, без wildcard CORS
- **Git, прочитанный вашим же клиентом** - ветка, изменения, лог и дифф файлов карточки читаются запуском
  `git`; машина без него отвечает «Git клиент недоступен» вместо падения. Aiko репозиторий только читает -
  единственный файл, который он правит, это `.gitignore` при регистрации проекта с политикой local-only, - и
  никогда не коммитит: коммиты делает агент по политике коммитов проекта
...
В конце он подключает найденных на машине агентов (`aiko agent install --scope user`): глобальные
скиллы `/aiko-*`, команды и правила пишутся им и больше никому (MCP-эндпоинт у каждого проекта свой и
подключается командой `aiko agent install --project <id>`). Назвать агентов вместо этого:
...
настройки сохраняются. Для удаления выполните `aiko uninstall`: команда останавливает демон, убирает
автозапуск и установленные файлы и удаляет `%LOCALAPPDATA%\Aiko\bin` из пользовательского
`PATH`; каталоги `.aiko` и база никогда не удаляются автоматически.
