...
- **Kanban PWA** на Blazor WebAssembly ([Flare.Blazor](https://github.com/jrfrigat/Flare)):
  проекции доски (по одной на тип карточек плюс Combined), экран бэклога, редактор карточек, артефакты
...
- **Типы карточек, которые вы определяете сами** - тип карточки *и есть* workflow: добавьте `Epic` с
...
- **Наборы workflow, которые можно создавать** - пайплайны и значения по умолчанию, с которых стартует
  проект: снимаются с проекта, экспортируются и импортируются между машинами и применяются к существующему
  проекту явным действием
...
Ставятся командой `aiko agent install`. Проектные (`--project <id>`):
...
| `/aiko-complete` | Завершить этап с файлами и артефактами |
| `/aiko-memory` | Записать и найти память проекта |
| `/aiko-status` | Сводка, что сейчас в работе |
| `/aiko-ui` | Открыть доску этого проекта |

Глобальные (`--scope user`), работают без открытого проекта:

| Скилл | Что делает |
| :-- | :-- |
| `/aiko-init` | Зарегистрировать текущий каталог как проект |
