...
Шестое — `Release`, второе действие, которое не называет карточку: оно просит агента провести выпуск
проекта, и выполняет его процедура `/aiko-release`. Версия, которую человек держит в голове, едет в `text`,
и процедура читает её прежде, чем выбрать версию сама: пустой текст — допустимая просьба, потому что выбор
и объяснение выбора — часть процедуры. Как и проход по доске, это одна запись, а не по одной на этап, и во
втором открытом `Release` проект отказывает: два выпуска сразу — это два тега для одного дерева. То, что
экран показывает рядом, читается из `GET /api/v1/projects/{projectId}/release` — версия самого демона,
последний выпуск, найденный через редирект `releases/latest` репозитория, и коммит, на котором стоит дерево
проекта.

Сам выпуск — это данные проекта в трёх местах, и каждое отвечает на свой вопрос:

- **Как проводится выпуск** — это *схема выпуска*: именованная инструкция агенту, с описанием и телом —
  порядком шагов. Схемы живут в секции `release` настроек проекта, и разрешается эта секция так же, как
  любая другая: проектное значение важнее шаблонного, шаблонное — то, с чего новый проект начинается, а
  поставляемое отвечает за проект, который не заявляет ничего. С Aiko поставляются две схемы:
  `git-release` — обычный выпуск, где тег несёт версию и выпуск становится последним, — и `git-pre-release`,
  тот же порядок с тегом, несущим `-pre`, который последним не становится. `SafeDefault` несёт обе, поэтому
  они есть у любого проекта, включая созданные до появления схем. Своего порядка процедура `/aiko-release`
  не называет: она читает схему, которой следует проект, и ведёт по её шагам.
