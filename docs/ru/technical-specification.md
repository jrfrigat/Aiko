...
Пять действий. Четыре отображаются на операцию, которая у агента уже есть: `Start` (`aiko_start_stage`),
...
| UI | Страница карточки - *Команда агенту*: поставить и прочитать, что с ней стало - ждёт, взята или отчёт агента. Доска - *Выполнить доску*: ставит команду `RunBoard` и показывает, что с ней стало |
